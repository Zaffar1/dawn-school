...
@section('title', 'Record ' . $config['name'])

@section('content')
<div class="page-title-box">
    <div>
        <h3 class="mb-1"><i class="fa-solid fa-plus me-2 text-primary"></i>Record {{ $config['name'] }}</h3>
...
<div class="card-box" style="max-width: 800px;">
    <h5 class="text-primary mb-4"><i class="{{ $config['icon'] }} me-2"></i>New Transaction Details</h5>

    <form method="POST" action="{{ route('hostel.store', $category) }}">
        @csrf
        ...
        <div class="row g-3">
            ...
            <div class="col-md-6">
                ...
                <div class="input-group">
                    ...
                    <input type="number" step="0.01" name="amount" class="form-control @error('amount') is-invalid @enderror" placeholder="0.00" value="{{ old('amount') }}" required>
                </div>
                @error('amount')
            ...
            <div class="col-md-6">
                ...
                <input type="text" name="reference_no" class="form-control @error('reference_no') is-invalid @enderror" 
                       ...
                       value="{{ old('reference_no') }}" {{ $category === 'electric-bill' ? 'required' : '' }}>
                @error('reference_no')
                    <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            </div>
            ...
            <div class="col-md-6">
                ...
                <input type="number" name="units_consumed" class="form-control @error('units_consumed') is-invalid @enderror" placeholder="e.g. 450" value="{{ old('units_consumed') }}">
            ...
            <div class="col-12">
                ...
                <textarea name="notes" rows="3" class="form-control @error('notes') is-invalid @enderror" placeholder="Provide any details about the payment, vendor, or reasons...">{{ old('notes') }}</textarea>
                @error('notes')
                    <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            </div>
