...
@section('title', 'Edit Fee Payment Record')

@section('content')
<div class="page-title-box">
    <div>
        <h3 class="mb-1"><i class="fa-solid fa-pen-to-square me-2 text-primary"></i>Edit Fee Payment Record</h3>
...
<div class="card-box" style="max-width: 800px;">
    <h5 class="text-primary mb-4"><i class="fa-solid fa-file-invoice me-2"></i>Payment Details</h5>

    <form method="POST" action="{{ route('hostel.resident-fees.update', $payment->id) }}">
        @csrf
        @method('PUT')
        ...
        <div class="row g-3">
            ...
            <div class="col-md-6">
                ...
                <select name="hostel_resident_id" id="hostel_resident_id" class="form-select @error('hostel_resident_id') is-invalid @enderror" required>
                    ...
                    @foreach($residents as $res)
                        <option value="{{ $res->id }}" 
                            {{ old('hostel_resident_id', $payment->hostel_resident_id) == $res->id ? 'selected' : '' }}
            ...
            <div class="col-md-6">
                ...
                <div class="p-2 border rounded bg-light" style="min-height:38px;">
                    <span id="resident_summary_info" class="text-muted small">Loading details...</span>
                </div>
            </div>
            ...
            <div class="col-md-6">
                ...
                <div class="input-group">
                    ...
                    <input type="number" step="0.01" name="due_amount" id="due_amount" class="form-control @error('due_amount') is-invalid @enderror" placeholder="0.00" value="{{ old('due_amount', $payment->due_amount ?? ($payment->resident->monthly_fee ?? 0.00)) }}" required>
                </div>
                @error('due_amount')
            ...
            <div class="col-md-6">
                ...
                <div class="input-group">
                    <span class="input-group-text bg-light text-muted">Rs.</span>
                    <input type="number" step="0.01" name="amount" id="amount" class="form-control @error('amount') is-invalid @enderror" placeholder="0.00" value="{{ old('amount', $payment->amount) }}" required>
                </div>
                @error('amount')
                    <div class="invalid-feedback d-block">{{ $message }}</div>
                @enderror
            ...
            <div class="col-md-6">
                ...
                <div class="input-group">
                    <span class="input-group-text bg-light text-muted">Rs.</span>
                    <input type="number" step="0.01" name="arrears" id="arrears" class="form-control @error('arrears') is-invalid @enderror" placeholder="0.00" value="{{ old('arrears', $payment->arrears ?? 0.00) }}" readonly>
                </div>
                @error('arrears')
                    <div class="invalid-feedback d-block">{{ $message }}</div>
                @enderror
            ...
            <div class="col-md-6">
                ...
                <input type="date" name="date" class="form-control @error('date') is-invalid @enderror" value="{{ old('date', $payment->date->toDateString()) }}" required>
                @error('date')
                    <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            </div>
            ...
            <div class="col-md-6">
                ...
                <input type="month" name="billing_month" class="form-control @error('billing_month') is-invalid @enderror" value="{{ old('billing_month', $payment->billing_month) }}" required>
                @error('billing_month')
                    <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            </div>
            ...
            <div class="col-md-6">
                ...
                <input type="text" name="reference_no" class="form-control @error('reference_no') is-invalid @enderror" placeholder="e.g. Bank slip, EasyPaisa transaction ID" value="{{ old('reference_no', $payment->reference_no) }}">
                @error('reference_no')
                    <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            </div>
            ...
            <div class="col-12">
                ...
                <textarea name="notes" rows="3" class="form-control @error('notes') is-invalid @enderror" placeholder="Provide any extra details about the payment...">{{ old('notes', $payment->notes) }}</textarea>
                @error('notes')
                    <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            </div>
            ...
            <div class="col-12 mt-4 d-flex gap-2">
                <button type="submit" class="btn btn-primary"><i class="fa-solid fa-floppy-disk me-1"></i> Update Payment</button>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        function updateResidentDetails(isInitial = false) {
            ...
            if (selectedOption && selectedOption.value !== '') {
                ...
                // Auto-fill ONLY if changing selection manually
                if (!isInitial) {
        ...
        residentSelect.addEventListener('change', function() {
            updateResidentDetails(false);
        });

        // Run on load
        updateResidentDetails(true);
    });
</script>
@endsection
